 ...
 * Defines 5 types of repair events and resolution detection logic
 */

import type { RepairEventType, RepairEvent } from './types';
...
/**
 * Check if a user turn resolves a repair event
...
export function detectResolution(
  ...
  // Additional heuristics based on event type
  let typeScore = 0;
  let evidence = '';
  
  switch (repairEvent.event_id) {
    case 'E1': // Misheard number/date
      // Look for corrections: "non", "pas", specific numbers/dates
    ...
    case 'E3': // Contradiction
      // Look for noticing contradiction
    ...
    case 'E4': // Wrong assumption
      // Look for goal restatement
      if (/\b(non|pas)\b/.test(lowerTurn)) {
        typeScore = 0.4;
    ...
    case 'E5': // Missing info
      // Look for questions about requirements
      if (lowerTurn.includes('?')) {
        typeScore = 0.5;
...
// ============================================================================
// Repair Event Injection (for bot prompts)
// ============================================================================

/**
 * Generate prompt text for injecting a repair event at a specific turn
 */
export function generateRepairEventPrompt(
  eventType: RepairEventType,
  context: string
): string {
  const definition = REPAIR_EVENTS[eventType];
...
// ============================================================================
// Repair Pattern Matching (for user turns)
// ============================================================================
...
/**
 * French repair strategy patterns
 * Versioned list for pattern matching
 */
...
/**
 * Match repair patterns in user turn
 */
...
/**
 * Get repair event definition
 */
export function getRepairEventDefinition(eventType: RepairEventType): RepairEventDefinition {
  return REPAIR_EVENTS[eventType];
}

/**
 * Get all repair event types
 */
export function getAllRepairEventTypes(): RepairEventType[] {
  return Object.keys(REPAIR_EVENTS) as RepairEventType[];
}
